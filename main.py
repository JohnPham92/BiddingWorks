...
def get_location_auctions(main_url: str) -> list:
    ...
    location_auction_urls = [f'{main_url}' + ana["href"] for ana in soup.findAll('a') if ana.parent.name == 'h2']
...
def retrieve_auction_location_items(location_auction_url: str) -> list:
    ...
    for thumb in auctions_page_soup.findAll('div', attrs={"class": "thumbpadding"}):
        ...
        item_auction_time = thumb.find('div', attrs={"class": "price auctions-time"}).text
        auction_location_items.append(
            [location_name, item_name, item_current_bid, item_auction_time, item_link, CURRENT_TIMESTAMP])
...
def write_to_csv_html(all_auction_location_items: list):
    ...
    """
    header = ['loc_name', 'item_name', 'item_current_bid', 'item_auction_time', 'item_link', 'scraped_at_local']
...
def check_run_program():
    ...
    for auction_end_date in df['auction_end_date'].unique():
        ...
        if (auction_end_date.date == CURRENT_TIMESTAMP.date) and (CURRENT_TIMESTAMP.hour in HOURS_TO_RUN):
            return True
...
def main():
    ...
    for location_auction_url in location_auction_urls:
        sleep(randint(1, 4))
        all_auction_location_items.extend(retrieve_auction_location_items(location_auction_url))
